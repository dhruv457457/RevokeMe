...
export const useAutoRevoke = () => {
  ...
  // A ref to hold the latest version of our watcher function
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          if (attempt === 3) {
            throw new Error('Failed to fetch gas prices after retries.');
          }
  ...
  const checkForUnseenApprovals = useCallback(async () => {
    ...
    try {
      ...
      const allActiveApprovals: Approval[] = (data?.data?.Approval ?? []).filter(app => {
        try { return BigInt(app.amount) > 0n; } catch { return false; }
      });
      ...
      if (approvalToRevoke) {
        ...
        try {
          const success = await triggerAutoRevoke(approvalToRevoke);
          if (success) {
            // No processedIds tracking needed
          }
        } finally {
  ...
  const authorizeAutoRevoke = async () => { /* ... unchanged ... */ };
  const revokeAuthorization = () => { /* ... unchanged ... */ };
