...
const RevokeConfirmationDialog: React.FC<Props> = ({ isOpen, isRevoking, approval, batchCount, onClose, onConfirm, isSuccess, txHash }) => {
        ...
        >
          ...
          >
            ...
            <div className="relative z-10 text-center">
              ...
              {isSuccess ? (
                <>
                  <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border-2 border-green-500 bg-green-900/30 mb-6">
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <h3 className="text-2xl font-bold mb-2">Revoke Successful!</h3>
                  <p className="text-green-400 font-mono text-sm break-all">{`Tx: ${txHash}`}</p>
                  {txHash && (
                    <a
                      href={`https://testnet.monadexplorer.com/tx/${txHash}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-400 hover:text-blue-300 transition-colors text-sm mt-4 inline-block"
                    >
                      View on Explorer
                    </a>
                  )}
                  <div className="mt-8">
                     <button
                      onClick={onClose}
                      className="px-8 py-3 w-36 rounded-3xl cursor-pointer font-semibold bg-[#333336] hover:bg-[#444447] transition-colors"
                    >
                      Close
                    </button>
                  </div>
