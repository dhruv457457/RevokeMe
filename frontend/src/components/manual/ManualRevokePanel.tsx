...
import {
  fetchApprovalsByAddress,
  type Approval,
} from "../../utils/fetchApprovals";
...
// --- Main Component ---
const ManualRevokePanel: React.FC<ManualRevokePanelProps> = ({
  onDataFetched,
}) => {
  const { address: eoaAddress } = useAccount();
  const { smartAccount, pimlicoClient, smartClient, isReady } =
    useSmartAccount();
  const { data: writeContractHash, writeContractAsync, isPending: isEoaTxPending } = useWriteContract();

  // Core State
  const [accountType, setAccountType] = useState<"smart" | "eoa">("eoa");
  const selectedAddress =
    accountType === "smart" ? smartAccount?.address : eoaAddress;
  ...
  const [eoaTxPurpose, setEoaTxPurpose] = useState<'deploy' | 'revoke' | null>(null);

  // Dialog and Revoke State
  ...
  const refetchApprovals = useCallback(() => {
    ...
    fetchApprovalsByAddress(selectedAddress)
      ...
      .catch((err) => {
        ...
        setStatus({ type: "error", message: "Could not refresh approvals." });
      })
  ...
  const handleConfirmRevoke = async () => {
    const approvalsToRevoke = isBatchConfirm ? approvals.filter(a => checkedIds.includes(a.id)) : (selectedApproval ? [selectedApproval] : []);
    if (approvalsToRevoke.length === 0) return;

    setIsRevoking(true);
    setStatus({ type: 'loading', message: `Revoking ${approvalsToRevoke.length} approval(s)...` });
    ...
    try {
        ...
        if (accountType === 'smart') {
            ...
            const calls = approvalsToRevoke.map(a => ({
            ...
            setStatus({ type: 'loading', message: 'Waiting for transaction...' });
            const { receipt } = await pimlicoClient.waitForUserOperationReceipt({ hash: opHash });
            txHash = receipt.transactionHash;
            setRevokeTxHash(txHash);
            refetchApprovals(); // Direct refetch for SA after await
        } else {
             setEoaTxPurpose('revoke'); // Set purpose before sending
             if (isBatchConfirm) {
                 ...
                 // In a real batch scenario, you'd get one hash from a multicall contract
                 txHash = 'Batch transactions sent.' 
             } else {
                txHash = await writeContractAsync({
                    address: getAddress(approvalsToRevoke[0].tokenAddress),
                    abi: erc20Abi,
                    functionName: 'approve',
                    args: [getAddress(approvalsToRevoke[0].spender), 0n]
                });
             }
             setRevokeTxHash(txHash);
        }
        ...
        setStatus({ type: 'error', message: e?.shortMessage || 'Revoke failed.' });
        closeDialogAndReset();
    } finally {
        setIsRevoking(false);
    }
  ...
  const {
    ...
    isLoading: isCheckingModule,
    refetch: refetchModuleAddress,
  } = useReadContract({
  ...
  useEffect(() => {
    if (isTxSuccess) {
      if (eoaTxPurpose === 'deploy') {
        setStatus({
          type: "success",
          message: "Module created successfully! You can now batch revoke.",
        });
        refetchModuleAddress();
      } else if (eoaTxPurpose === 'revoke') {
        // Successful revoke, refetch the list
        refetchApprovals();
      }
      setEoaTxPurpose(null); // Reset purpose after handling
    }
  }, [isTxSuccess, eoaTxPurpose, refetchModuleAddress, refetchApprovals]);
  ...
  const handleCreateModule = async () => {
    try {
      setStatus({
        type: "loading",
        message: "Deploying your personal revocation module...",
      });
      setEoaTxPurpose('deploy'); // Set purpose
      await writeContractAsync({
        address: revocationModuleFactoryAddress,
        abi: revocationModuleFactoryAbi,
      ...
      setStatus({
        type: "error",
        message: e?.shortMessage || "Module creation failed.",
      });
    }
  };

  const getButtonState = () => {
    const isLoadingState = isRevoking || isEoaTxPending;
    if (accountType === "eoa") {
      if (isCheckingModule)
        return { text: "Checking Setup...", disabled: true, action: () => {} };
      if (!userModuleAddress || userModuleAddress === zeroAddress)
        return {
          text: "Enable Batch Revoke",
          disabled: isLoadingState,
          action: handleCreateModule,
        };
    }
    return {
      text: `Revoke Selected (${checkedIds.length})`,
      disabled: checkedIds.length === 0 || isLoadingState,
      action: () => {
      ...
      <div className="flex-shrink-0">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-4 border-b border-gray-700 pb-4">
          ...
          <div className="flex items-center gap-2 p-1 bg-[#1A1A1D] rounded-lg w-full md:w-auto">
            <button
              onClick={() => handleAccountTypeChange("eoa")}
              className={`w-full md:w-auto px-3 py-1.5 rounded-md text-sm font-semibold transition-colors ${
                accountType === "eoa"
            ...
            <button
              onClick={() => handleAccountTypeChange("smart")}
              className={`w-full md:w-auto px-3 py-1.5 rounded-md text-sm font-semibold transition-colors ${
                accountType === "smart"
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <div className="relative" ref={filterRef}>
              <button
                ...
                className="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm bg-[#1A1A1D] border border-[#333336] rounded-md text-gray-300 "
              >
                <span>Filters</span>
                <span className="text-xs bg-gray-600 px-1.5 py-0.5 rounded-full">
                  {activeFilters.amount.length + activeFilters.type.length}
                </span>
              </button>
              {isFilterOpen && (
                <div className="absolute top-full right-0 mt-2 w-60 bg-[#0C0C0E] border border-[#333336] rounded-lg shadow-xl z-20 p-4 text-gray-300 text-sm">
                  <h5 className="font-bold text-gray-400 text-xs uppercase tracking-wider mb-2">
                    Asset Type
                  </h5>
                  <label className="flex items-center gap-2 mb-3">
                    <input
                      ...
                      checked={activeFilters.type.includes("token")}
                      onChange={() => handleFilterChange("type", "token")}
                    />{" "}
                    Token
                  </label>
                  <label
                    className="flex items-center gap-2 mb-3 cursor-not-allowed text-gray-600"
                    title="NFT data not available"
                  >
                    <input type="checkbox" disabled /> NFT
                  </label>

                  <h5 className="font-bold text-gray-400 text-xs uppercase tracking-wider mb-2 border-t border-gray-700 pt-3">
                    Approved Amount
                  </h5>
                  <label className="flex items-center gap-2 mb-3">
                    <input
                      type="checkbox"
                      className="accent-purple-500"
                      checked={activeFilters.amount.includes("unlimited")}
                      onChange={() => handleFilterChange("amount", "unlimited")}
                    />{" "}
                    Unlimited
                  </label>
                  <label className="flex items-center gap-2 mb-3">
                    <input
                      type="checkbox"
                      className="accent-purple-500"
                      checked={activeFilters.amount.includes("limited")}
                      onChange={() => handleFilterChange("amount", "limited")}
                    />{" "}
                    Limited
                  </label>
                </div>
            ...
            <div className="relative" ref={sortRef}>
              <button
                ...
                className="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm bg-[#1A1A1D] border border-[#333336] rounded-md text-gray-300"
              >
                <span>Sort</span> <ChevronDownIcon />
              </button>
              {isSortOpen && (
                <div className="absolute top-full right-0 mt-2 w-64 bg-[#0C0C0E] border border-[#333336] rounded-lg shadow-xl z-20 text-gray-300 text-sm">
                  {sortOptions.map((opt) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 hover:bg-[#1A1A1D] ${
                        sortOption === opt.key ? "bg-[#1A1A1D]" : ""
                      }`}
                    >
                      {opt.label}
                    </button>
      ...
      <div className="flex-grow overflow-y-auto">
        {isLoading ? (
          ...
          <ApprovalsTable
            ...
            isLoading={isRevoking || isEoaTxPending}
          />
        )}
      </div>
        ...
        {approvals.length > 0 && !isLoading && (
          <div className="flex items-center justify-between gap-4">
            <button
              ...
              className="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-bold disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors flex items-center justify-center min-w-[180px]"
            >
              {(isRevoking || isEoaTxPending) &&
              buttonState.text !== "Checking Setup..." ? (
                <IconSpinner />
              ) : (
                buttonState.text
              )}
